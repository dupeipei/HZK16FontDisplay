...
def writeWords(rect_list):
    ...
    for i in range(len(rect_list)):
        for j in range(len(rect_list[i])):
            if rect_list[i][j]:
                # 前景字符（即用来表示汉字笔画的输出字符）
                plt.scatter(colGap * j + int(j / 16) * colGap, -rowGap * i, color='darkred', s=circleSize)
            else:
                # 背景字符（即用来表示背景的输出字符）
                plt.scatter(colGap * j + int(j / 16) * colGap, -rowGap * i, color='k', s=circleSize, alpha=0.1)

    # scatter:x,y表示横纵坐标，color表示颜色：'r':红  'b'：蓝色 等，marker:标记，edgecolors:标记边框色'r'、'g'等，s：size大小
    # plt.scatter(x,y, color='b', marker='o', edgecolors='g', s=200)

    wordSize = len(rect_list)
    wordCount = wordSize / 16
    figRow = colGap * wordSize + (wordCount-1) * colGap
    figCol = wordSize
    plt.axis('off')  # 不显示坐标轴
    plt.show()
...
# inputWords = input()
inputWords = "范卓轩"
rect_list = printPlay(inputWords)
writeWords(rect_list)